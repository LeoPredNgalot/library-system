...
// ── Navigation ──
// Accepts either a string id ('home', 'books' …) OR a DOM element directly.
// Links that have no id (Profile Settings) pass `this` so we get the element.
function setActiveNav(elOrId) {
  document.querySelectorAll('.nav-link').forEach(l => l.classList.remove('active'));
  const el = typeof elOrId === 'string'
    ? document.getElementById('nav-' + elOrId)
    : elOrId;
...
// clickedEl is optional — pass `this` from onclick for links without an id
function navigateTo(pageKey, title, url, clickedEl) {
  // clickedEl wins; fall back to id-based lookup (nav-books, nav-users …)
  setActiveNav(clickedEl || pageKey);
